...
const source = await readFile(resolve(process.cwd(), "contracts/TypeMark.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "TypeMark.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["TypeMark.sol"]?.TypeMark;
if (!contract) {
...
console.log(`Deploying TypeMark from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_TYPE_MARK_CONTRACT_ADDRESS to this contract address in Vercel.");
